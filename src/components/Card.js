...
const Card = ({ version, date, label, labelColor, author, authorImage, description }) => {
  return (
    <div className="bg-white shadow-lg rounded-lg p-6 flex flex-col h-full">
      ...
      <p className="text-gray-700 text-base mb-4 whitespace-pre-line flex-grow">{description}</p>
      <div className="mt-auto">
        <button className="bg-white text-indigo-500 border border-indigo-500 px-4 py-2 rounded-full w-32" style={{ borderRadius: '100vmax' }}>
          Download
        </button>
      </div>
    </div>
